...
export default function History({ shifts, setShifts, hourlyRate, currency }) {
  ...
  const handleAddManualShift = () => {
    ...
    const updatedShifts = [newShift, ...shifts].sort((a, b) => b.startTime - a.startTime);
  ...
  const totalEarned = shifts.reduce((sum, shift) => sum + shift.earned, 0).toFixed(2);
    ...
    <div className="p-6 h-full flex flex-col animate-fade-in">
      <div className="flex justify-between items-end mb-6 bg-white/[0.02] p-5 rounded-3xl border border-white/5">
        <div>
          ...
          {/* ИСПРАВЛЕНИЕ: Отделили иконку от прозрачного текста */}
      ...
      <div className="flex justify-between items-center mb-4 px-1">
        ...
        <button 
          ...
          className={`p-2 rounded-xl transition-colors ${isManualEntryOpen ? 'bg-rose-500/20 text-rose-400' : 'bg-indigo-500/20 text-indigo-400 hover:bg-indigo-500/30'}`}
        >
      ...
      {isManualEntryOpen && (
        <div className="bg-indigo-500/10 p-5 rounded-3xl border border-indigo-500/20 mb-4 animate-fade-in flex flex-col gap-4">
          <h4 className="text-xs text-indigo-300 font-bold uppercase tracking-widest">Добавить вручную</h4>
          <input 
            type="date" 
            value={manualDate}
            onChange={(e) => setManualDate(e.target.value)}
            className="bg-black/40 text-white border border-white/5 rounded-xl py-3 px-4 focus:outline-none focus:border-indigo-500 w-full color-scheme-dark"
          />
          <div className="flex gap-3 items-center">
            <input 
              ...
              className="bg-black/40 text-white border border-white/5 rounded-xl py-3 px-4 focus:outline-none focus:border-indigo-500 flex-1 color-scheme-dark"
            />
          ...
          <button 
            ...
            disabled={!manualDate || !manualStartTime || !manualEndTime}
            className="w-full bg-indigo-600 hover:bg-indigo-700 disabled:bg-gray-800 disabled:text-gray-500 text-white font-bold py-3 rounded-xl transition-colors mt-2"
          >
            Сохранить смену
          </button>
        </div>
      ...
      <div className="flex-1 overflow-y-auto space-y-3 pb-24 no-scrollbar">
        {shifts.length === 0 ? (
          ...
          shifts.map((shift, i) => (
            <div key={shift.id} className="bg-white/[0.03] hover:bg-white/[0.06] transition-colors p-5 rounded-3xl border border-white/5 flex flex-col gap-3" style={{ animationDelay: `${i * 30}ms` }}>
