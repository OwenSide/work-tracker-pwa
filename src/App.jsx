...
import { useIndexedDB } from './db'; // <-- Путь изменен сюда
...
export default function App() {
  ...
  // Подключаем наши модули IndexedDB
  ...
  // Ждем, пока IndexedDB асинхронно отдаст все данные
  ...
  // Экран загрузки, пока база данных инициализируется
  if (!isAppReady) {
    return (
          ...
          <nav className="pointer-events-auto bg-gray-900/80 backdrop-blur-2xl border border-white/10 flex justify-around p-2 rounded-3xl shadow-[0_-10px_40px_rgba(0,0,0,0.5)] max-w-sm mx-auto">
            {[
              ...
              <button
                ...
                className={`flex flex-col items-center justify-center p-3 rounded-2xl transition-all duration-300 w-24 relative
                  ${activeTab === tab.id 
                    ? 'text-white bg-white/10 scale-100' 
                    : 'text-gray-500 hover:text-gray-300 scale-95 hover:bg-white/5'}`}
              >
                <tab.icon size={22} className={`mb-1.5 transition-transform duration-300 ${activeTab === tab.id ? 'scale-110 text-indigo-400' : ''}`} />
